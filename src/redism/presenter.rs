...
use super::state::{
    RedisConnectionDefinition, RedisHashState, RedisJsonState, RedisListState, RedisLocalState,
    RedisSetsState, RedisSortedSetsState,
};

struct RedisPresenter;

impl RedisPresenter {
    #[inline(always)]
    pub fn read_operation(m: &str, result: RedisResult<Value>) -> String {
        match result {
            Ok(rresp) => {
                format!("{m} :: {pr}", pr = redis_value_to_string(&rresp))
            }
            Err(err) => format!("ERROR {m} :: {err:?}"),
        }
    }
}
...
impl JsonPresenter {
    ...
    pub fn json_type(conn: &mut redis::Connection, st: &mut RedisJsonState) -> String {
        RedisPresenter::read_operation(
            "JSON.TYPE",
    ...
    pub fn json_arrlen(conn: &mut redis::Connection, st: &mut RedisJsonState) -> String {
        RedisPresenter::read_operation(
            "JSON.ARRLEN",
    ...
    pub fn json_arrindex(conn: &mut redis::Connection, st: &mut RedisJsonState) -> String {
        let (start, stop) = (
        ...
        RedisPresenter::read_operation("JSON.ARRINDEX", result)
    }
    ...
    pub fn json_mget(conn: &mut redis::Connection, st: &mut RedisJsonState) -> String {
        let ks = st.json_mget_ks.split(' ').collect::<Vec<&str>>();
        JsonPresenter::_json_get(conn, "JSON.MGET", &ks, &st.json_mget_p)
    }

    pub fn json_get(conn: &mut redis::Connection, st: &mut RedisJsonState) -> String {
        JsonPresenter::_json_get(conn, "JSON.GET", &[&st.json_get_k], &st.json_mget_p)
    }

    pub fn json_objlen(conn: &mut redis::Connection, st: &mut RedisJsonState) -> String {
        RedisPresenter::read_operation(
            "JSON.OBJLEN",
    ...
    pub fn json_objkeys(conn: &mut redis::Connection, st: &mut RedisJsonState) -> String {
        RedisPresenter::read_operation(
            "JSON.OBJKEYS",
    ...
    pub fn json_strlen(conn: &mut redis::Connection, st: &mut RedisJsonState) -> String {
        RedisPresenter::read_operation(
            "JSON.STRLEN",
    ...
    // ==================================================
    #[inline(always)]
    pub fn _json_get(conn: &mut redis::Connection, m: &str, k: &[&str], p: &str) -> String {
        RedisPresenter::read_operation(m, conn.json_get(k, p))
    }
    ...
    fn _write_json_operation(
        ...
        match result {
            Ok(rresp) => {
                ...
                format!("{m} :: {rr}", rr = redis_value_to_string(&rresp))
            }
            Err(err) => {
                format!("ERR {m} :: {err:?}")
            }
