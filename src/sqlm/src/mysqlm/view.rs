...
pub struct MySqlView {
    ...
    generator_window: GeneratorWindow<MySqlRow, MySql, MySqlType>,
    insertion_window: InsertionWindow<MySqlRow, MySql, MySqlType>,
}

impl Default for MySqlView {
    fn default() -> Self {
...
impl MySqlView {
    pub fn update(
        ...
        egui::CentralPanel::default().show(ctx, |ui| {
            ...
            let mut sql_layouter = |ui: &egui::Ui, string: &str, wrap_width: f32| {
                let mut layout_job =
                    egui_extras::syntax_highlighting::highlight(ui.ctx(), &ui.style(), &theme, string, "sql");
        ...
        if self.state.sql.data_gen.table_to_generate_data.is_some() {
            ...
            if self.state.sql.data_gen.show_generator_window {
                let pr = SqlPresenter::<MySql>::default();
    ...
    fn statement_filter(&self, row_idx: usize) -> Vec<String> {
        let sql_presenter = SqlPresenter::<MySql>::default();
        ...
        self.state
            ...
            .iter()
            .enumerate()
            .filter(|(_, e)| sql_presenter.should_be_added_to_delete_stmt(&e.1))
            .map(|(col_idx, e)| {
                format!(
                    "{} = {}",
                    e.0,
                    ...
                    ui.with_layout(egui::Layout::right_to_left(egui::Align::LEFT), |ui| {
                        ...
                        if ui.button("Update").clicked() {
                            ...
                            self.run_statement(
                                ...
                                rt,
                                stmt,
                                !pg_app_state.performance_table,
                                true,
                            );
                            self.state.sql.row_being_editted.selected_row = None;
                        }
