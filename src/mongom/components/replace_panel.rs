...
impl MongoView {
    pub fn replace(&mut self, rt: &Runtime, ctx: &egui::Context, i18n: &I18n) {
        let filter: Document =
        ...
        if doc.is_empty() || filter.is_empty(){
            return;
        }
        ...
        rt.spawn(async move {
            let result =
                presenter::replace(&tx, &i18n_cloned, &client, &db_name, &col_name, doc, &filter)
                    .await;
